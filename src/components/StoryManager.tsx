...
import { Globe, Plus, Trash2, Edit3, Heart, Save, X, Search, Star, MapPin } from 'lucide-react';
import { SupabaseService } from '../lib/supabase';
import { cn } from '../lib/utils';

interface DiasporaStory {
  id: string;
  name: string;
  location: string;
  story: string;
...
export const StoryManager = () => {
  ...
  const fetchStories = async () => {
    setLoading(true);
    try {
      const data = await SupabaseService.getStories();
      setStories(data || []);
    } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      if (currentStory.id) {
        await SupabaseService.updateStory(currentStory.id, currentStory);
      } else {
        await SupabaseService.createStory(currentStory as any);
      }
      setIsEditing(false);
      fetchStories();
    } catch (err) {
    ...
    <div className="space-y-8 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        {!isEditing && (
          <button 
            onClick={() => {
              setCurrentStory({ category: 'Success Story', isFeatured: false });
              setIsEditing(true);
            }}
              ...
              <div key={story.id} className="bg-white rounded-[40px] border border-slate-100 p-8 hover:shadow-xl transition-all group overflow-hidden relative">
                ...
                <div className="flex items-center justify-between">
                   ...
                   <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button onClick={async () => { if(confirm('Sûr ?')) { await SupabaseService.deleteStory(story.id); fetchStories(); } }} className="p-2 text-slate-400 hover:text-red-500"><Trash2 size={16}/></button>
                   </div>
                </div>
              </div>
            ))}
