...
export const UserProfileTabs = ({ 
  ...
  const handleVerifyMFA = async () => {
    ...
    try {
      ...
      await SupabaseService.enable2FA(user.uid, 'totp');
  ...
  const handlePINChange = async () => {
    ...
    try {
      await SupabaseService.setPIN(user.uid, pin);
  ...
  const handleKYCSubmit = async () => {
    ...
    try {
      // 1. Upload ID Card
      const idUrl = await SupabaseService.uploadFile('kyc', `${user.uid}/id_card_${Date.now()}`, kycFiles.id_card);
      // 2. Upload Selfie
      const selfieUrl = await SupabaseService.uploadFile('kyc', `${user.uid}/selfie_${Date.now()}`, kycFiles.selfie);
      ...
      // 4. Notify Admin
      await SupabaseService.notifyAdminKYC(user.uid, user.displayname);
    ...
    <div className="space-y-6 md:space-y-10 animate-in slide-in-from-right-4 duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
        <div className="bg-slate-50 rounded-3xl p-6 md:p-8 space-y-6">
           ...
           <button 
            onClick={user.two_factor_enabled ? () => SupabaseService.disable2FA(user.uid).then(() => setFormData(p => ({...p, two_factor_enabled: false}))) : handleEnrollMFA}
            className={cn(
